...
export const updateUserDataFromGoogle = async (googleData: GoogleAuthInterface, userId: number) => {
  const connection = await getDatabaseConnection();
  const { name, picture } = googleData;

  const { firstName, lastName } = splitGoogleName(name);

  await connection.manager.update(User, { id: userId }, { firstName, lastName, profilePicture: picture })
